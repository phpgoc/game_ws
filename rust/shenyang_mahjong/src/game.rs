...
pub(crate) fn allow_chi(configs: &HashMap<String, i32>) -> bool {
    win_rule_from_configs(configs) != WIN_RULE_SHENYANG_BASIC
}
...
pub(crate) fn build_claim_options(
    ...
    let allow_chi_now = allow_chi(configs);
    ...
    for position in positions {
        ...
        let chi_options = if allow_chi_now && position == next_position {
...
fn capped_winner_hand_fan(
    ...
    let fan = winner_hand_fan_with_rule(state, settlement, winner, win_rule_from_configs(configs));
...
pub(crate) fn resolve_claim_window(
    ...
    let invalid_rob_gang_tile_count =
        is_rob_gang && has_impossible_known_tile_count(state, claim_window.tile);
    ...
    for position in &claim_window.eligible_positions {
        ...
        match claim_window.responses.get(position) {
            Some(ClaimResponse::Hu) => {
                ...
                tiles.sort_unstable();
                if !invalid_rob_gang_tile_count
            ...
            if state.claim_window.is_some() {
                ...
                let response = match payload.action {
                    ...
                    ShenyangMahjongAction::HU => {
                        if is_rob_gang && has_impossible_known_tile_count(&state, claim_tile) {
                            return room_service.error_response(
                                session_id,
                                Routes::PLAY as i32,
                                WsResponseCode::NO_PERMISSION,
                    ...
                    ShenyangMahjongAction::CHI => {
                        ...
                        if !allow_chi(&configs)
                            || position != state.next_position(from_position)
...
mod tests {
    ...
    #[test]
    fn claim_options_hide_chi_for_shenyang_basic_rule() {
        let mut state = playable_state();
        state
            .hands
    ...
    #[test]
    fn play_request_chi_rejects_shenyang_basic_rule() {
