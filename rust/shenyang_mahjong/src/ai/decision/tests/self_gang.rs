...
fn dealer_self_gang_preserves_basic_four_pairs_missing_suit_seven_pairs_plan() {
    ...
    assert_eq!(choose_self_gang_from_view(&hand, &[1], &table, 0), None);
}
...
fn dealer_takes_unready_concealed_gang_that_preserves_committed_piao() {
    ...
    assert_eq!(best_ready_score_after_discard(&hand, melds, &table, 0), 0.0);
    assert_eq!(choose_self_gang_from_view(&hand, &[21], &table, 0), None);
...
fn non_dealer_delays_closed_dragon_gang_but_dealer_takes_replacement_draw() {
    ...
    assert_eq!(choose_self_gang_from_view(&hand, &[35], &table, 0), None);
    assert_eq!(choose_self_gang_from_view(&hand, &[35], &table, 0), None);

    table.dealer_position = 0;
    assert_eq!(
        choose_self_gang_from_view(&hand, &[35], &table, 0),
        Some(35)
    );
    assert_eq!(
        choose_self_gang_from_view(&hand, &[35], &table, 0),
...
fn self_gang_delays_closed_plain_gang_before_ready_without_opening() {
    ...
    assert_eq!(best_ready_score_after_discard(&hand, &[], &table, 0), 0.0);
    assert_eq!(choose_self_gang_from_view(&hand, &[3], &table, 0), None);
    assert_eq!(choose_self_gang_from_view(&hand, &[3], &table, 0), None);
...
fn self_gang_allows_committed_piao_plan_when_gang_reaches_piao_ready() {
    ...
    assert!(ready_has_piao_win(&after_gang, &after_melds, &table, 0));
    assert_eq!(
        choose_self_gang_from_view(&hand, &[21], &table, 0),
        Some(21)
    );
...
fn self_gang_ignores_invalid_candidate_tile() {
    ...
    assert_eq!(
        choose_self_gang_from_view(&hand, &[3, 35], &table, 0),
        Some(3)
    );
    assert_eq!(choose_self_gang_from_view(&hand, &[35], &table, 0), None);
}
...
fn self_gang_passes_committed_piao_plan_when_gang_only_reaches_basic_ready() {
    ...
    assert!(ready_tile_score(&after_gang, &after_melds, &table, 0) > 0.0);
    assert!(!ready_has_piao_win(&after_gang, &after_melds, &table, 0));
    assert_eq!(choose_self_gang_from_view(&hand, &[21], &table, 0), None);
}
...
fn self_gang_passes_final_unready_hand_for_defense() {
    ...
    assert_eq!(best_ready_score_after_discard(&hand, &[], &table, 0), 0.0);
    assert_eq!(choose_self_gang_from_view(&hand, &[3, 35], &table, 0), None);
}
...
fn self_gang_passes_pure_plan_when_gang_only_reaches_basic_ready() {
    ...
    assert!(best_ready_score_after_discard(&hand, current_melds, &table, 0) > 0.0);
    assert!(ready_tile_score(&after_gang, &melds, &table, 0) > 0.0);
    assert!(!ready_has_pure_one_suit_win(&after_gang, &melds, &table, 0,));
    assert_eq!(choose_self_gang_from_view(&hand, &[1], &table, 0), None);
}
...
fn self_gang_rejects_incomplete_virtual_hand() {
    ...
    assert_eq!(
        choose_self_gang_from_view(&[35, 35, 35, 35], &[35], &table, 0),
        None
    );
}
...
fn self_gang_rejects_public_fifth_copy() {
    ...
    assert_eq!(
        choose_self_gang_from_view(&concealed_hand, &[35], &concealed_table, 0),
        None
    );
    ...
    assert_eq!(
        choose_self_gang_from_view(&added_hand, &[35], &concealed_table, 0),
        None
    );
}
...
fn self_gang_stops_preserving_four_gui_yi_against_threatening_dealer() {
    ...
    assert!(!dealer_opponent_has_major_threat(&table, 0));
    assert_eq!(
        choose_self_gang_from_view(&keeps_ready, &[31], &table, 0),
        None
    );
    ...
    assert!(best_ready_score_after_discard(&keeps_ready, melds, &table, 0,) > 0.0);
    let current_score = best_score_after_forced_discard(&keeps_ready, melds, &table, 0);
    let gang_score = self_gang_score(31, &keeps_ready, melds, &table, 0, current_score);
    ...
    assert_eq!(
        choose_self_gang_from_view(&breaks_ready, &[31], &table, 0),
        None
    );
}

#[test]
fn self_gang_treats_chi_as_opening_meld() {
    let mut table = table_with_discards(1, Vec::new());
    table.seats.get_mut(&0).unwrap().melds = vec![test_chi_meld(1)];
    let melds = table.seats.get(&0).unwrap().melds.as_slice();
    let hand = vec![11, 12, 13, 21, 22, 23, 31, 35, 35, 35, 35];
