...
fn piao_discard_bias_protects_live_dragon_pair() {
    ...
    let middle_pair = piao_discard_bias(&hand, 11, &[], &table, 0);
    let dragon_pair = piao_discard_bias(&hand, 35, &[], &table, 0);
    assert_eq!(piao_discard_bias(&hand, 35, &[], &table, 0), dragon_pair);
...
fn piao_discard_bias_protects_live_pair_over_dead_pair() {
    ...
    let dead_pair = piao_discard_bias(&hand, 11, &[], &table, 0);
    let live_pair = piao_discard_bias(&hand, 21, &[], &table, 0);
