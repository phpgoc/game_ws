...
pub(in crate::ai::decision) fn should_peng_to_preserve_four_gui_yi_from_discard(
    ...
    let gang_ready_score = ready_tile_score(&gang_hand, &gang_melds, table, position);
    if gang_ready_score <= 0.0 {
        return false;
    }
    if let Some(score_cap) = table.score_cap.filter(|score_cap| *score_cap > 0)
        && ready_hand_visible_fan_reaches_cap(&gang_hand, &gang_melds, table, position, score_cap)
    {
        return false;
    }
    ...
    unique_tiles(&peng_hand).into_iter().any(|discard| {
        ...
        estimated_four_gui_yi_fan(&after_discard, &peng_melds) > gang_four_gui_yi
            ...
            && ready_tile_score_after_discard(&after_discard, &peng_melds, table, position, discard)
                >= gang_ready_score
    })
}
